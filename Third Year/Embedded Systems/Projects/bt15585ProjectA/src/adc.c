...
/* ---------------------------------------
		readingVoltage function:
			Calls MeasureVoltage function and converts
			the readings taken into a float number. 
   --------------------------------------- */

void readingVoltage(void){
	MeasureVoltage();
	voltage = (VREF*sres) / ADCRANGE;
}
/* ---------------------------------------
    Take one measurement of the ADC input
